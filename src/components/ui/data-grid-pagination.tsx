...
function DataGridPagination(props: any) {
  ...
  const to = Math.min((pageIndex + 1) * pageSize, safeRecordCount);
    ...
    >
      {/* <div className="flex flex-wrap items-center space-x-2.5 pb-2.5 sm:pb-0 order-2 sm:order-1">
        {isLoading ? (
          ...
          <>
            <div className="text-sm text-muted-foreground">ROWS_PER_PAGE</div>
            <Select
              value={`${pageSize}`}
          ...
          <>
            ...
            {pageCount > 1 && (
              <div className="flex items-center space-x-1 order-1 sm:order-2">
                ...
                >
                  <span className="sr-only">{t('ui.goToPreviousPage')}</span>
                ...
                >
                  <span className="sr-only">{t('ui.goToNextPage')}</span>
