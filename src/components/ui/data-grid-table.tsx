...
function DataGridTableHeadRowCell({ children, header, dndRef, dndStyle }: any) {
  const { props } = useDataGrid();
...
function DataGridTableBodyRowCell({ children, cell, dndRef, dndStyle }: any) {
  const { props } = useDataGrid();
...
function DataGridTableEmpty() {
  ...
  return (
    <tr>
      ...
      >
        {props.emptyMessage || t('common.noDataAvailable')}
      </td>
    </tr>
  );
}
  ...
  return (
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
      <div className="text-muted-foreground bg-card  flex items-center gap-2 px-4 py-2 font-medium leading-none text-sm border shadow-xs rounded-md">
        ...
        {props.loadingMessage || t('ui.loading')}
      </div>
    </div>
  );
}
